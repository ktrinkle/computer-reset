...
import { Signup, StateList, CityList, TimeslotSmall, openEvent, CountryList, UserSmall, UserRetrieve, frontPage } from '../data';
...
@Component({
  ...
  styleUrls: ['./privateevent.component.scss']
})
export class PrivateeventComponent  implements OnInit, OnDestroy {
  ...
  ngOnInit() {
      ...
      this.activatedRoute.paramMap.subscribe(params => {
        let eventKeyString = params.get('eventKey');
        this.eventKey = Guid.parse(eventKeyString);
        if (Guid.isGuid(eventKeyString))
        {
          this.signUp.eventKey = eventKeyString;
        }
  ...
  loadEvents() {
    ...
    this.dataService.getPrivatePage(this.eventKey, this.userLoad).subscribe({next: (data: frontPage)=>{
      ...
      this.events.forEach((event, index) => {
        ...
        // this is because we will only have one on this component
        this.eventForm.patchValue({eventId: event.id});
        this.moveOrSignup = event.userSlot != null;
  ...
  async eventSubmit() {
    ...
    if (Guid.validator.test(this.eventKey.toString()))
    {
      this.signUp.eventKey = this.eventKey.toString();
    }
